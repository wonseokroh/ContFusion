...
class GraphFusion(Base3DFusionModel):
    def __init__(
        self,
        ...
        **kwargs,
        ...
        if decoder.get('img') is not None:
            self.decoders['img'] = nn.ModuleDict(
                {
                    "backbone": build_backbone(decoder['img']["backbone"]),
                    "neck": build_neck(decoder['img']["neck"]),
                }
            )
        if decoder.get('pts') is not None:
            self.decoders['pts'] = nn.ModuleDict(
                {
                    "backbone": build_backbone(decoder['pts']["backbone"]),
                    "neck": build_neck(decoder['pts']["neck"]),
                }
            )
    ...
    def forward(
        ...
        **kwargs,
        ...
        # disc_loss = -1 * self.disc_loss(disc_pred[0], self.disc_gt)

        # del cam_feat, lidar_feat, disc_feats, disc_pred
        ...
        # conv_layer = torch.nn.Conv2d(80, 256, 1, stride=1, padding=0,).cuda()
        # camera_tsne_feat = conv_layer(camera_tsne_feat)
        ...
        # if tsne_features_np.shape[0] >= 135:
        #     # print('============ draw tsne ============')
        ...
        #     plt.savefig('./feature_disc_bev_bar.png')
        #     raise ValueError
        #     ######################## Distibution ########################
        #     # raise ValueError
        ...
        #     x = features[0]
        # print(f'x len: {len(x)} || x 0: {x[0].shape} ')
        # batch_size = x.shape[0]
        # [256, 180, 180]

        # x = self.decoders['pts']["backbone"](x)
        # x = self.decoders['pts']["neck"](x)
        # print(f'x len: {len(x)} || x 0: {x[0].shape} ')
        # Before || x len: 1 || torch.Size([1, 512, 180, 180]) 
        ...
        img_feat = self.decoders['img']["neck"](img_feat)

        pts_feat = self.decoders['pts']["backbone"](features[1])
        pts_feat = self.decoders['pts']["neck"](pts_feat)
