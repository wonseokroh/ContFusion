...
from mmdet3d.models.utils import FFN, PositionEmbeddingLearned, GraphTransformerDecoderLayer
...
class GraphFusionHead(nn.Module):
    def __init__(
        ...
        super(GraphFusionHead, self).__init__()
    ...
    def forward_single(self, inputs, img_inputs, metas):
        ...
        Args:
            inputs (torch.Tensor): Input feature map with the shape of
                ...
                torch.Size([512, 180, 180])
        Returns:
            list[dict]: Output results for tasks.
        """
        batch_size = inputs.shape[0]
        lidar_feat = self.shared_conv_lidar(inputs)
        image_feat = self.shared_conv_image(img_inputs)
        ...
        bev_pos_pts = self.bev_pos.repeat(batch_size, 1, 1).to(lidar_feat.device)

        image_feat_flatten = image_feat.view(
            batch_size, image_feat.shape[1], -1
        )  # [BS, C, H*W]
        bev_pos_img = self.bev_pos.repeat(batch_size, 1, 1).to(image_feat.device)
        ...
        # query_feat: torch.Size([1, 128, 200])  ||  lidar_feat_flatten: torch.Size([1, 128, 32400])
        ...
        query_pos = bev_pos_pts.gather(
            index=top_proposals_index[:, None, :]
            .permute(0, 2, 1)
            .expand(-1, -1, bev_pos_pts.shape[-1]),
            dim=1,
        )
        ...
        for i in range(self.num_decoder_layers):
            ...
            query_feat = self.decoder[i](query_feat, image_feat_flatten, query_pos, bev_pos_img)
    ...
    def forward(self, feats, metas):
        ...
        if isinstance(feats[0], torch.Tensor):
            lidar_feat = [feats[0]]
        if isinstance(feats[1], torch.Tensor):
            image_feat = [feats[1]]    
        lidar_feat, image_feat = feats

        res = multi_apply(self.forward_single, lidar_feat, image_feat, [metas])
