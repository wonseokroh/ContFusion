...
__all__ = ["PositionEmbeddingLearned", "GraphTransformerDecoderLayer", "GraphMultiheadAttention", "FFN"]
...
class GraphTransformerDecoderLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation="relu",
        ...
        if not self.cross_only:
            self.self_attn = GraphMultiheadAttention(d_model, nhead, dropout=dropout, cross=False)
        self.multihead_attn = GraphMultiheadAttention(d_model, nhead, dropout=dropout, cross=True)
        # self.multihead_attn = DGCNNAttn(d_model, nhead, dropout=dropout)
...
class GraphMultiheadAttention(nn.Module):
    ...
    Shape:
        ...
        if hasattr(self, '_qkv_same_embed_dim') and self._qkv_same_embed_dim is False:
            return graph_multi_head_attention_forward(
...
def edge_feats(query, key):
    # (B, N, N)
    affinity = torch.cdist(query, key)
    affinity = torch.sigmoid(affinity)
   
    return affinity

def graph_multi_head_attention_forward(query,  # type: Tensor
                                 key,  # type: Tensor
                                 value,  # type: Tensor
    ...
    if cross:
        ...
        ########### GraphFusion #############
        adjacency = edge_feats(q, k)
        # # Query torch.Size([8, 200, 16]) || Key torch.Size([8, 200, 16]) torch.Size([8, 32, 200, 16])
        neigh_node = torch.bmm(adjacency, k)
        # # print(q.shape, k.shape, adjacency.shape, neigh_node.shape)
        attn_output_weights = torch.bmm(q, neigh_node.transpose(1, 2))
        
        # torch.Size([8, 200, 16]) torch.Size([8, 200, 16]) torch.Size([8, 200, 200]) torch.Size([8, 200, 200])
...
class DGCNNAttn(BaseModule):
    ...
    def forward(self,
        ...
        **kwargs allow passing a more general data flow when combining
        ...
        edge_feats = self.edge_feats(query, key, K=self.K)
    ...
    def edge_feats(self, query, key=None, K=10):
        # (B, N, N)
        if key is not None:
            affinity = torch.cdist(query, key.transpose(0,1))
        else:
